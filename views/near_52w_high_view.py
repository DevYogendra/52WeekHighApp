...
from db_utils import get_all_dates, get_data_for_date, get_historical_market_cap
...
def main():
    ...
    st.caption("Leadership names near highs. Use this page to quickly rank them by valuation, quality, and market-cap trend.")
    ...
    if date_mode == "Single Date":
        ...
        with col2:
            if range_method == "Presets":
                ...
                num_days = st.number_input("Days", min_value=1, value=7)
                start_date_default = max_date_available - relativedelta(days=num_days - 1)
            else:
                num_months = st.number_input("Months", min_value=1, value=3)
    ...
    with grouped_tab:
        ...
        selected_group_industry = st.selectbox("Grouped industry", grouped_industries, key="near_high_grouped_industry")
        ...
        render_interactive_table(
            ...
            key=f"near_high_group_{selected_group_industry}",
            rename_map=COMMON_RENAME_MAP,
    ...
    st.caption("Tip: use column filters and multi-sort in the table headers for quick valuation and quality screens.")
